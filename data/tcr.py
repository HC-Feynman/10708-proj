...
class TCR(nn.Module):
    def __init__(self):
        ...
        self.ang = np.deg2rad(5.0)  # 20 # Change the degree of rotation as per the task in hand
        self.ang_neg = -1 * self.ang
        self.max_tx, self.max_ty = 2.0, 2.0  # 6.0, 6.0  # Change as per the task
        self.min_tx, self.min_ty =  -2.0, -2.0 # -6.0, -6.0  # Change as per the task

        self.max_z, self.min_z = 0.5, 0.5 # 1.00, 1.00  # Change as per the task
